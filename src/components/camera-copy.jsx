...
const Camera = () => {
  ...
  useEffect(() => {
    ...
    async function main() {
      ...
      const gripThreshold = 0.3; // 手を握った判定の閾値（適宜調整）
      ...
      const interval = setInterval(async () => {
        ...
        predictions.forEach((prediction) => {
          ...
          const thumbMiddleDist = Math.sqrt(Math.pow(thumbTip[0] - middleTip[0], 2) + Math.pow(thumbTip[1] - middleTip[1], 2));

          if (thumbMiddleDist < gripThreshold) {
            console.log('手を握りました');
            setPose('back');
            prevX = null;
            prevY = null;
